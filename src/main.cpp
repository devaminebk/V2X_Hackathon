...
// --- Thresholds & Timers ---
const int DISTANCE_THRESHOLD_CM = 100; // Kid detected if closer than 100cm
const float MOVEMENT_THRESHOLD = 0.5;  // Acceleration threshold (m/s^2) for X/Y axis
unsigned long lastSendTime = 0;
const int SEND_COOLDOWN_MS = 2000;     // Prevent spamming ESP-NOW messages (2 seconds)
...
bool isBusStationary() {
    ...
    // Check if X and Y acceleration are within the quiet threshold
    if (abs(a.acceleration.x) < MOVEMENT_THRESHOLD && 
        abs(a.acceleration.y) < MOVEMENT_THRESHOLD) {
        return true;
    }
    return false;
}
