...
typedef struct struct_message {
    uint8_t msg_type;
    uint32_t payload;
    uint8_t risk_score; // AI risk score from cloud (carried in Step-1, ignored in Steps 2&3)
} struct_message;

// State tracking variables
volatile bool isVerifying = false;
volatile bool alertVerified = false; 
volatile bool alertVerified2 = true;
unsigned long verifyStartTime = 0;
uint8_t busMac[6];
uint8_t receivedRiskScore = 60; // AI: stores risk score for current alert
...
void triggerLegitimateWarning() {
  ...
  // AI: show risk score screen after the warning sequence
  display.clearDisplay();
  display.setCursor(0, 0);
  display.setTextSize(1);
  display.println(" ZONE RISK:");
  display.println("------------");
  display.setTextSize(2);
  display.setCursor(10, 20);
  display.print(receivedRiskScore);
  display.print("%");
  display.setTextSize(1);
  display.setCursor(0, 38);
  if (receivedRiskScore >= 90)      display.println(" HIGH RISK");
  else if (receivedRiskScore >= 60) display.println("  CAUTION");
  else                              display.println(" LOW RISK");
  display.display();
  delay(2000);
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (incoming.msg_type == MSG_STEP_1 && !isVerifying) {
      ...
      // AI: capture the risk score carried in this packet
      receivedRiskScore = incoming.risk_score;
...
void loop() {
  ...
  if (isVerifying && !alertVerified && !alertVerified2) {
